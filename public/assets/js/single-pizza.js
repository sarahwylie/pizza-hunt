...
function getPizza() {
  ...
  fetch(`/api/pizzas/${pizzaId}`)
    .then(response => {
      if (!response.ok) {
        throw new Error({ mesage: 'Eep! Something went awry!' })
      }
    ...
    .catch(err => {
      console.log(err);
      alert('Cannot find a pizza with this ID - taking you back in time.')
      window.history.back();
    })
}
...
function handleNewCommentSubmit(event) {
  ...
  fetch(`/api/comments/${pizzaId}`, {
    ...
    .then(response => {
      if (!response.ok) {
        throw new Error('Oh no! Something has gone awry!');
      }
      response.json();
    })
...
function handleNewReplySubmit(event) {
  ...
  fetch(`/api/comments/${pizzaId}/${commentId}`, {
    ...
    .then(response => {
      if (!response.ok) {
        throw new Error('Oh no! Something has gone horribly awry!');
      }
      response.json();
    })
    .then(commentResponse => {
